"""
Defines a hugo site within the context of a Bazel workspace.

Required due to build issues within rules_hugo implementation
(e.g. `config_dir` is not a valid attribute for `hugo_site`).
"""
...
def copy_to_dir(ctx, srcs, dirname):
    ...
    Args:
        ctx: ctx from rule
        srcs: list of files to copy
        dirname: string of directory to copy to
    Returns:
        list of files
    """
    outs = []
